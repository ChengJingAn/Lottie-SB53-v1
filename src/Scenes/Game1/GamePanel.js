...
export default function Scene2({ finishGame, _baseGeo, _geo, stopSound }) {
    ...
    const layoutstonetPos = { x: -2.2, y: 0.28 }
    ...
    function reRenderingFunc() {
        ...
        if (currentNum % 10 == 0) {
            characterList[0].current.setPosInfo({
                l: layoutstonetPos.x + 0.06 + targetRange * currentNum + currentStep * foodAreaLength,
                b: layoutstonetPos.y + 0.22
            })
        }
        else {
            characterList[0].current.setPosInfo({
                ...
                setTimeout(() => {
                    for (let i = 1; i < 3; i++) {
                        characterList[i].current.setPosInfo({
                            l: layoutstonetPos.x + 0.05 +
                                targetRange * (currentNum - stepNumRange) + i * 0.06 + foodAreaLength * currentStep,
                            b: layoutstonetPos.y + 0.22 + stepRange * heightList[currentNum - stepNumRange - 1 + i]
                        })
                    }
                    ...
                    let interval = setInterval(() => {
                        ...
                        characterList[0].current.setPosInfo({
                            l: layoutstonetPos.x + 0.073 + targetRange * currentNum + foodAreaLength * currentStep,
                            b: layoutstonetPos.y + 0.22 + stepRange * heightList[currentNum - 1]
                        })
                        ...
                        if (currentNum % 10 == 0) {
                            ...
                            setTimeout(() => {
                                ...
                                for (let i = currentNum - 10; i < currentNum; i++) {
                                    stoneList[i].current.setUrl('SB_53_Prop-Interactive/SB53_stone_deactivated_01.svg')
    ...
    function showButtonAni(obj, num) {
        ...
        if (obj == redstone) {
            ...
            obj.current.style.bottom = (layoutstonetPos.y + 0.225 + heightList[num] * stepRange) * 100 + '%'
        }
        else {
            obj.current.style.left = (layoutstonetPos.x + 0.159 + num * targetRange + foodAreaLength * currentStep) * 100 + '%'
            obj.current.style.bottom = (layoutstonetPos.y + 0.225 + heightList[num] * stepRange) * 100 + '%'
        }
            ...
            <div
                ...
                {Array.from(Array(3).keys()).map(value =>
                    <BaseImage
                        ...
                        posInfo={{
                            l: layoutstonetPos.x + 0.06,
                            b: layoutstonetPos.y + 0.22
                        }}
                        url={'SB_53_Character_Animation/SB53_CI_hen_head move_0' + [value + 1] + '.svg'}
                    />
                )}
